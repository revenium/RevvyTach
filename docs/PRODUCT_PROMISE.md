...
- Chrome-assisted setup reads only profile labels when opening a profile. Manual **Read from Chrome** reads the selected profile's claude.ai session-key cookie after a consent notice. If claude.ai later refuses a saved browser sign-in, automatic recovery can reread its remembered Chrome profile or inspect each unclaimed Chrome profile's session-key cookie and check that key against claude.ai's organizations endpoint. Legacy pairing saves a key and Chrome source only for a unique match to a saved personal organization; shared, team, and unknown-classification organizations are excluded. Background reads do not require a separate press or consent notice for each candidate and may prompt for Chrome Safe Storage access. No other cookie is decrypted; login databases, saved passwords, and browser history remain non-goals. See `docs/data-and-privacy.md`.
