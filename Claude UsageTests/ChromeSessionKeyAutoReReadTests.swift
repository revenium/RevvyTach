...
//  profiles read their recorded source, legacy personal profiles require one
//  conclusive organization match, unchanged keys write nothing, and attempts are
...
final class ChromeSessionKeyAutoReReadTests: XCTestCase {
    ...
    private func makeReader(
        ...
        ChromeSessionKeyAutoReReader(
            ...
            saveSessionKey: { attempt in
                ...
                recorder.recordSave(attempt.sessionKey)
                return .stored
            },
    ...
    func testSharedOrganizationDoesNotPairAnotherMembersChromeSessions() async {
        let recorder = Recorder()
        let attemptLog = AttemptLog()
        let otherKey = "sk-ant-sid01-other0000000000000"
        let reader = makeDiscoveryReader(
            recorder: recorder,
            profiles: [
                ChromeProfile(name: "Member One", directoryName: "Default"),
                ChromeProfile(name: "Member Two", directoryName: "Profile 19"),
            ],
            readOutcomes: [
                "Default": .sessionKey(freshKey),
                "Profile 19": .sessionKey(otherKey),
    ...
    func testUnknownOrganizationClassificationDoesNotPair() async {
        let recorder = Recorder()
        let attemptLog = AttemptLog()
        let reader = makeDiscoveryReader(
            recorder: recorder,
    ...
    private func makeCompareAndSetReader(
        ...
        return ChromeSessionKeyAutoReReader(
            ...
            saveSessionKey: { attempt in
                guard profile.sessionKey == attempt.refusedSessionKey,
                      profile.source?.directoryName
                        == attempt.previousSource?.directoryName else {
                    return .superseded
                }
                profile.sessionKey = attempt.sessionKey
